...
$azureSQLDBDetails = Get-AzureRmSqlDatabase -ServerName $azureSQLServerDetails -ResourceGroupName RG-DockerDemo
...
#####################################################################
        ...
        $SQLServer_object = $null
        $SQLServer_object = @()

        foreach($azureSQLDBDetail_Iterator in $azureSQLDBDetails){
    
            # Populating the cells

            $SQLServer_object_temp = new-object PSObject

            $SQL_Server_object_temp | add-member -MemberType NoteProperty -Name "ResourceGroupName" -Value $azureSQLDBDetail_Iterator.ResourceGroupName
            $SQL_Server_object_temp | add-member -MemberType NoteProperty -Name "SQLServerName" -Value $azureSQLDBDetail_Iterator.ServerName
            $SQL_Server_object_temp | add-member -MemberType NoteProperty -Name "DatabaseName" -Value $azureSQLDBDetail_Iterator.DatabaseName
            $SQL_Server_object_temp | add-member -MemberType NoteProperty -Name "Location" -Value $azureSQLDBDetail_Iterator.Location
            $SQL_Server_object_temp | add-member -MemberType NoteProperty -Name "ElasticPool" -Value $azureSQLDBDetail_Iterator.ElasticPoolName
            $SQL_Server_object_temp | add-member -MemberType NoteProperty -Name "Status" -Value $azureSQLDBDetail_Iterator.Status

            
        
            # Setting the pointer to the next row and first column
            $SQLServer_object += $SQLServer_object_temp
    }

    $SQLServer_object | Export-Csv "SQLServer_Details.csv" -NoTypeInformation -Force
